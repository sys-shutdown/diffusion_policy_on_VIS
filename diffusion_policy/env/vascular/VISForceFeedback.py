...
class Obeserver(Sofa.Core.Controller):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.iteration = 0
        self.rootNode = kwargs.get("rootNode")
        self.data_queue = kwargs.get("data_queue")

    def onAnimateBeginEvent(self, dt): # 用来读数据的Callback
        self.iteration += 1
        if self.iteration % 5 == 0:
            data=dict(frame=self.iteration, position=self.rootNode.InstrumentCombined.DOFs.position.value, force=self.rootNode.InstrumentCombined.DOFs.force.value)
            self.data_queue.put(data)

def createScene(root,data_queue,
    ...
    guide.addObject('RodStraightSection', name="StraightSection", youngModulus="10000", nbEdgesCollis="50", nbEdgesVisu="196", length="380.0")
    ...
    instrument.addObject(Obeserver(name='observer',rootNode=root, data_queue=data_queue))
    ...
    instrument.addObject('InterventionalRadiologyController', template='Rigid3d', name='m_ircontroller', printLog=False, xtip=[10+config["insertion"], 0], step=3, rotationInstrument=[0,0],
    ...
    cath_visuOgl.addObject('OglModel', name='Visual', color=[0.1, 0.5, 0.9, 0.5], quads='@../ContainerCath.quads', material='texture Ambient 1 0.2 0.2 0.2 0.0 Diffuse 1 1.0 1.0 1.0 1.0 Specular 1 1.0 1.0 1.0 1.0 Emissive 0 0.15 0.05 0.05 0.0 Shininess 1 20')
    ...
    collision.addObject('MeshSTLLoader', name='meshLoader', filename=path+'visual_modified.stl', triangulate=True, flipNormals=True)
    collision.addObject('MeshTopology', position='@meshLoader.position', triangles='@meshLoader.triangles')
    collision.addObject('MechanicalObject', name='DOFs1',scale=1)
    ...
    collision.addObject('OglModel', name='Visual', src='@meshLoader', color=[1, 0, 0, 0.3], scale=1)
...
def observe(data_queue):
    from matplotlib import pyplot as plt
    from mpl_toolkits.mplot3d import Axes3D
    fig = plt.gcf()
    ax1 = fig.add_subplot(projection='3d',autoscale_on=False)
    plt.ion()
    while True:
        if not data_queue.empty():
            ...
            ax1.set_ylabel('Y')
            ax1.set_zlabel('Z')
            data=data_queue.get() # 读到的导丝上各节点位置量、旋转量在 data['position'] 里，前3位置向量，后4旋转四元数
            pos=data['position'][:,0:3]
            force=data['force']
            print(np.sum(force**2,axis=1))
            ax1.plot3D(pos[:,0],pos[:,1],pos[:,2], color='k')
            plt.pause(0.01)
        #line.remove()
...
if __name__ == '__main__':
    dataQueue = Queue()
    sim_process = Process(target=simulation,args=(dataQueue,))
    ob_process = Process(target=observe,args=(dataQueue,))
    sim_process.start()
    ob_process.start()
    sim_process.join()
    ob_process.join()